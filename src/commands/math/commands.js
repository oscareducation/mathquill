...
if (transformPropName) {
  scale = function (jQ, x, y, ty_pct = 0) {
    if (ty_pct !== 0)
      jQ.css(
        transformPropName,
        "scale(" + x + "," + y + ") translateY(" + ty_pct + "%)"
      );
    //added Y translation because rendering is too low
    else jQ.css(transformPropName, "scale(" + x + "," + y + ")");
  };
...
var SquareRoot = (LatexCmds.sqrt = LatexCmds["√"] = P(MathCommand, function (
  ...
  _.reflow = function () {
    ...
    scale(
      block.prev(),
      1,
      (block.innerHeight() * 1.2) / +block.css("fontSize").slice(0, -2) - 0.1,
      -10
    );
  };
}));
...
// hat and widehat templates are kind of hackish but good enough for now
var widehatSvg =
  '<svg width="30px" height="0.3em" viewBox="0 0 2364 300" preserveAspectRatio="none"><path d="M1181 0h2l1171 176c6 0 10 5 10 11l-2 23c-1 6-5 10 -11 10h-1L1182 67 15 220h-1c-6 0-10-4-11-10l-2-23c-1-6 4-11 10-11z"></path></svg>';
LatexCmds.widehat = bind(DiacriticAbove, "\\widehat", widehatSvg, [
  "widehatSvg(",
  ")",
]);
var hatTemplate =
  '<span style="line-height:0; vertical-align:bottom;">&#94;</span>';
LatexCmds.hat = bind(DiacriticAbove, "\\hat", hatTemplate, ["hat(", ")"]);
...
function DelimsMixin(_, super_) {
  ...
  _.reflow = function () {
    ...
    scale(this.delimjQs, min(1 + 0.2 * (height - 1), 1.2), 1.2 * height, -15);
  };
}
...
var Matrix = (Environments.matrix = P(Environment, function (_, super_) {
  ...
  _.reflow = function () {
    ...
    if (parens.length) {
      ...
      scale(parens, min(0.5 + 0.2 * (height - 1), 1.2), 1.05 * height);
    }
  };
  _.latex = function () {
    var latex = "";
  ...
  _.html = function () {
    ...
    this.htmlTemplate =
      ...
      '<table class="' +
      (this.centered == true ? "mq-cell-centered " : "") +
      'mq-non-leaf">' +
